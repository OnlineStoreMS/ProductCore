...
func (r *BrandRepo) List(keyword string) ([]model.Brand, error) {
	tx := r.db.Model(&model.Brand{}).Order("sort DESC, id ASC")
	if keyword != "" {
...
func (r *BrandRepo) GetByID(id uint64) (*model.Brand, error) {
	var b model.Brand
...
func (r *BrandRepo) Delete(id uint64) error {
	res := r.db.Delete(&model.Brand{}, id)
	if res.Error != nil {
		return res.Error
	}
...
func (r *BrandRepo) CountProducts(brandID uint64) (int64, error) {
	var n int64
	err := r.db.Model(&model.Product{}).Where("brand_id = ?", brandID).Count(&n).Error
	return n, err
}
...
func (r *CategoryRepo) ListAll() ([]model.Category, error) {
	...
	err := r.db.Order("sort DESC, id ASC").Find(&list).Error
	return list, err
}
...
func (r *CategoryRepo) CountChildren(parentID uint64) (int64, error) {
	var n int64
	err := r.db.Model(&model.Category{}).Where("parent_id = ?", parentID).Count(&n).Error
	return n, err
}

func (r *CategoryRepo) CountProducts(categoryID uint64) (int64, error) {
	var n int64
	err := r.db.Model(&model.Product{}).Where("category_id = ?", categoryID).Count(&n).Error
	return n, err
}
...
func (r *GroupRepo) List() ([]model.ProductGroup, error) {
	var list []model.ProductGroup
	err := r.db.Order("sort DESC, id ASC").Find(&list).Error
	return list, err
}

func (r *GroupRepo) GetByID(id uint64) (*model.ProductGroup, error) {
...
func (r *GroupRepo) CountProducts(groupID uint64) (int64, error) {
	var n int64
	err := r.db.Model(&model.ProductGroupRelation{}).Where("group_id = ?", groupID).Count(&n).Error
	return n, err
}

func (r *GroupRepo) ListProductIDs(groupID uint64) ([]uint64, error) {
	var ids []uint64
